...
<html>
	...
	<body>
		<?php
		if(isset($_POST['num_verger']))
		{
			$num_verger = $_POST['num_verger'];
			
			$verif = mysqli_query($base, 'SELECT CodeVerger FROM verger WHERE CodeVerger="'.$num_verger.'"');
			
				if(mysqli_num_rows($verif) == 1)
				{		
					if($req = mysqli_query($base, 'DELETE FROM verger WHERE CodeVerger="'.$num_verger.'"'))
					{
						echo 'Votre verger a bien été supprimé';
						header("Refresh: 1;URL=supprimer_verger.php");
					}
					else
					{
		...
		{	
			...
			if(isset($_SESSION['status']))
			{
				...
				?>
				<br/><br/> 
				<form action="supprimer_verger.php" method="POST">
					<label>Code du verger : </label><input type="text" name="num_verger" required /><br/><br/>
